...
def check_set_working_env():
    ...
    if options.ortho:
        ...
        mod_dir = pidgin_dir + sep + 'no_ortho' + sep
...
def get_Models():
    ...
    if options.ntrees: model_info = [m[:2]+[str(options.ntrees)]+m[3:] if idx > 0 else m for idx, m in enumerate(model_info)]
    ...
    if options.p_filt:
        ...
        try:
            ...
            print(' Filtering models for a minimum ' + metric + ' performance of ' + str(perf_threshold) + ' during ' + validation + ' validaiton')
            ...
            sys.exit()
    if options.organism:
        orgs = map(str.lower, options.organism.split(','))
    for row in uniprot_info:
        # Filter bioactivity/organism/targetclass/minsize/se/performance (if set)
        if options.bioactivity and row[8] not in options.bioactivity.split(','):
                    continue
        if options.organism and (row[4] == '' or not any([org.lstrip() in row[4].lower() for org in orgs])):
                    continue
        if options.targetclass and row[3] not in options.targetclass:
                    continue
        if sum(map(int,row[9:11])) < options.minsize:
                    continue
        if options.se_filt and int(row[13]) > 0:
                    continue
        if options.p_filt:
            try:
                            if float(model_info[row[-1]][train_row_idx].split(',')[0]) < perf_threshold:
                                continue
            except ValueError:
                            continue
